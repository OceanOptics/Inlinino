...
Inlinino is a simple data logger. It was developped to log measurments from scientific instruments during extended periods of time (months) but can be used or adapt to your need. It supports any instrument with serial or analog interface as long as there is an API for it or you can make one.
...
There is two interface available to date: a friendly `Graphical User Interface <gui.html>`__ also known as a `GUI <gui.html>`__ (default) and a `Command Line Interface (CLI) <cli.html>`__ for use on servers or power use.
